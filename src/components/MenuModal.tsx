...
import { FaHome, FaProjectDiagram, FaUserTie, FaTools, FaEnvelope } from "react-icons/fa";
      ...
      <div className="w-full h-full flex flex-col items-center justify-center">
        ...
        <nav className="flex flex-col items-center space-y-6 text-white text-lg font-medium">
          {[
            ...
            { name: "Projects", icon: <FaProjectDiagram />, href: "?tab=projects" },
            { name: "Experience", icon: <FaUserTie />, href: "?tab=experience" },
            { name: "Skills", icon: <FaTools />, href: "?tab=skills" },
            { name: "Connect", icon: <FaEnvelope />, href: "https://wa.me/9389968605" }, // Replace with actual WhatsApp number
          ].map((item, index) => (
            <a
              key={index}
              href={item.href}
